...
color_cuts_clean = np.logical_and((ri < iy - iyri_cut + rishift),
                             (iy > iy_cut + iyshift))
...
print(f'color cuts used for final optimal selection with no degraded y band: comp = {clean_comp:.2}, pure = {clean_pure:.2}')
noise_new_comp, noise_new_pure = complete_purity_cutsonly(color_cuts_noise_new, photoz, zcut = [1.05, 1.55])
print(f'ri < iz + {izri_noise_cut:.2} & iz > {iz_noise_cut:.2} & iy_noise > {iy_noise_cut:.2}: completeness = {noise_new_comp:.2}, purity = {noise_new_pure:.2}')
...
sd0 = ax0.scatter(ri[color_cuts_clean], iy[color_cuts_clean], c = prob[color_cuts_clean] , cmap = 'viridis', s=3, alpha =.1, vmin=0, vmax=1)
sd1 = ax1.scatter(ri[color_cuts_clean], iz[color_cuts_clean], c = prob[color_cuts_clean], cmap = 'viridis', s=3, alpha =.1, vmin=0, vmax=1)
sd2 = ax2.scatter(iy[color_cuts_clean], iz[color_cuts_clean], c = prob[color_cuts_clean], cmap = 'viridis', s=3, alpha =.1, vmin=0, vmax=1)

sd3 = ax3.scatter(ri[color_cuts_noise_new], iy_noise[color_cuts_noise_new], c = prob[color_cuts_noise_new] , cmap = 'viridis', s=3, alpha=.1, vmin=0, vmax=1)
sd4 = ax4.scatter(ri[color_cuts_noise_new], iz[color_cuts_noise_new], c = prob[color_cuts_noise_new], cmap = 'viridis', s=3, alpha=.1, vmin=0, vmax=1)
sd5 = ax5.scatter(iy_noise[color_cuts_noise_new], iz[color_cuts_noise_new], c = prob[color_cuts_noise_new] , cmap='viridis', s=3, alpha=.1, vmin=0, vmax=1)
...
ax0.set_xlim(*ri_lim)
ax0.set_ylim(*iy_lim)
ax0.axhline(y = iy_cut + iyshift, xmax = 0.35, ls = '--', c = 'black') 
...
ax1.set_xlim(*ri_lim)
ax1.set_ylim(*iz_lim)
ax1.set_title(f'color cuts used for final optimal selection with no degraded y band: comp = {clean_comp:.2}, pure = {clean_pure:.2}', fontsize = 15)
...
#colors cut and plotted with y noise
ax3.set_xlabel('r-i', fontsize = 27)
ax3.set_ylabel(f'i-y$_{{noise}}$', fontsize = 27)
ax3.set_xlim(*ri_lim)
ax3.set_ylim(*iy_lim)
ax3.axhline(y = iy_noise_cut, ls = '--', c = 'black') 
...
ax4.set_xlim(*ri_lim)
ax4.set_ylim(*iz_lim)
ax4.axhline(y = iz_noise_cut, xmax = 0.48, ls = '--', c = 'black')
x_noise = np.arange(iz_noise_cut, 2, .05)
y_noise = x_noise + izri_noise_cut
ax4.plot(y_noise , x_noise , ls = '--', c = 'black')
ax4.set_title(f'r-i < i-z + {izri_noise_cut:.2} & i-z > {iz_noise_cut} & i-y$_{{noise}}$ > {iy_noise_cut:.2}: comp= {noise_new_comp:.2}, pure = {noise_new_pure:.2}', fontsize = 15)
...
ax5.axvline(x = iy_noise_cut, ymin = iz_noise_cut + 0.10, ls = '--', color='black')
...
plt.savefig('/Users/yokisalcedo/Desktop/Emission-Line-Galaxy-Target-Selection/script_figure/yband_noise_colorcolor_cut.png', dpi=300, bbox_inches='tight')