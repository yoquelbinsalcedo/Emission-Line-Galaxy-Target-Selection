...
def plot_and_stats(z_spec, z_phot, ylabel, isvert = True, vlinemin = 1.05, vlinemax = 1.65, filepath = '/Users/yokisalcedo/Desktop/Emission-Line-Galaxy-Target-Selection/script_figure/spec_vs_phot_z_cosmos_ez.png'):
...
colormaskx = np.logical_and.reduce((np.isfinite(cat['photoz']),
                                    ...
                                    (np.logical_or(cat['HSC_g_MAG']< 24.5, 
                                    cat['HSC_r_MAG']< 24.5))))
cat = cat[colormaskx]
...
photoz_cos = combined_cat_cos['ez_z_phot']
...
plot_and_stats(specz_cos[snr_mask_comb_cos], photoz_cos[snr_mask_comb_cos], ylabel = 'Cosmos2020 EAZY photoz', isvert = True)
