...
def thresbins_iy(data, ri, iy, vlinemin, vlinemax,
                  filepath = '/Users/yokisalcedo/Desktop/Emission-Line-Galaxy-Target-Selection/script_figure/iyshifts.png',
                    xlabel = 'LePhare Photometric Redshift',ylabel = 'Count' ,histtype = 'step',xlim=(0, 2),**kwargs):
    ...
    for pthres in threshold:
        mask = np.logical_and((iy - 0.19 > ri) , iy > 0.35 + pthres)
        _, _, ptemp = ax.hist(data[mask], bins = np.linspace(0, 2, 100) , histtype = histtype, label = f'i-y > 0.35 + {pthres:0.02f} & i-y - 0.19 > r-i  ', **kwargs)
        p.append(ptemp)
...
thresbins_iy(specz,ri, 
             iy,vlinemin = 1.05,
             vlinemax = 1.65,
             filepath = '/Users/yokisalcedo/Desktop/Emission-Line-Galaxy-Target-Selection/script_figure/iyshifts.png', 
             xlabel = '$z_{\mathrm{spec}}$')
